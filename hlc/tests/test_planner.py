...
def test_one_obstacle_6x6():
    ...
    for key, value in all_positions.items():
        if key == (
                1, 2):  # pos with obsticle, value needs to be false = pos not visited
            assert value != True
...
def test_multiple_obsticle_6x6():
    ...
    for key, value in all_positions.items():
        if key == (
                0, 2):  # pos with obsticle, value needs to be False = pos not visited
            assert value != True
        elif key == (1, 5):
            assert value != True
        elif key == (3, 4):
...
def test_dead_end_6x6():
    ...
    for key, value in all_positions.items():
        if key == (
                1, 1):  # pos with obsticle, value needs to be false = pos not visited
            assert value != True
        elif key == (1, 2):
            assert value != True
        elif key == (1, 3):
